...
void TerrainGrid::Brush(const Point2f& position, float radius, float valueChange)
{
	...
	v *= (1.f / m_size);
	int index{ std::round(v.x) + std::round(v.y) * m_resolution };
	if (index > 0 and index < m_resolution*m_resolution) {
...
Point2f MC::InterpolateGridPoints(const GridPoint& p1, const GridPoint& p2, float threshold)
{
	...
	return (Vector2f{p1.m_pos} +  Vector2f{ p1.m_pos,p2.m_pos } * (threshold - p1.m_value) / (p2.m_value - p1.m_value)).ToPoint2f();
}
